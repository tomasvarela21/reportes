"""
Copiado de services/file_parser.py — no importa desde fuera de /backend.
Parser de archivos CSV del Libro Diario. Soporta Formato A (22 cols) y B (17 cols).
"""
import io
import logging
...
class FileParser:
    ...
    def _procesar_df_comun(self, df: pd.DataFrame, resultado: ParseResult) -> ParseResult:
        ...
        if df['debe'].isna().any():
            resultado.errores.append(
                f"{df['debe'].isna().sum()} fila(s) con valor no numérico en 'debe'.")
        if df['haber'].isna().any():
            resultado.errores.append(
                f"{df['haber'].isna().sum()} fila(s) con valor no numérico en 'haber'.")
