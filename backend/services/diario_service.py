...
def upload_csv(
    ...
    # Convertir claves "YYYY/MM" → (int, int)
    ...
    for k, v in periodos_raw.items():
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Clave de período inválida: '{k}'. Usar formato YYYY/MM")
    ...
    if not result.ok:
        raise HTTPException(status_code=422, detail={"errores": result.errores, "advertencias": result.advertencias})
