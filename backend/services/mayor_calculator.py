"""
Adaptado de services/mayor_calculator.py — no importa desde fuera de /backend.
Motor de cálculo del Libro Mayor acumulado. Siempre requiere conn externo.
"""
import logging
import time
...
EMPRESAS_INV = {1: 'BATIA', 2: 'NORFORK', 3: 'GUARE', 4: 'TORRES', 5: 'WERCOLICH'}
...
class MayorCalculator:
    ...
    def recalcular(self, empresa_id: int, desde_anio: int, desde_mes: int, motivo: str = "manual") -> int:
        inicio = time.time()
        empresa_nombre = EMPRESAS_INV.get(empresa_id, str(empresa_id))
        log.info(f"Recálculo — empresa={empresa_nombre} desde={desde_anio}/{desde_mes:02d}")
        ...
        try:
            ...
            if not periodos:
                log.warning(f"Sin períodos en libro_diario para empresa_id={empresa_id}")
                return 0
    ...
    def _calcular_periodo(self, cur, empresa_id: int, anio: int, mes: int, saldos_previos: dict) -> int:
        ...
        if not todas_las_claves:
            cur.execute("DELETE FROM libro_mayor WHERE empresa_id=%s AND periodo_anio=%s AND periodo_mes=%s",
                        (empresa_id, anio, mes))
            return 0
