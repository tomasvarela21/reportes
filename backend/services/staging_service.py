"""
Adaptado de services/staging_service.py — no importa desde fuera de /backend.
Staging y carga del Libro Diario. Siempre requiere conn externo.
"""
import logging
import time
...
class StagingService:
    ...
    def ejecutar_carga_multiperiodo(
        ...
        try:
            ...
            for anio, mes in periodos:
                ...
                df_mes = df[
                    (df['periodo_anio'] == anio) & (df['periodo_mes'] == mes)
                ].copy()
            ...
            registros_mayor = calc.recalcular(
                ...
                desde_mes=mes_desde,
                motivo=f"api_carga_{len(periodos)}_periodos",
            )
    ...
    def _cargar_periodo(self, cur, df_mes: pd.DataFrame, empresa_id: int,
                        anio: int, mes: int, archivo_nombre: str, reemplazar: bool) -> tuple:
