...
def cargar_apertura_csv(conn, df, anio_fiscal, archivo_nombre):
    ...
    try:
        for emp in empresas_en_archivo:
            cur.execute("DELETE FROM saldos_apertura WHERE empresa=%s AND anio_fiscal=%s", (emp, anio_fiscal))
        registros = []
        for _, row in df.iterrows():
            registros.append((
                str(row['empresa']), int(anio_fiscal), int(row['cuenta_codigo']),
                str(row.get('tipo_subcuenta','')) or None, str(row.get('nro_subcuenta','')) or None,
                str(row.get('centro_costo','')) or None, float(row['saldo']),
                datetime.now(), archivo_nombre,
            ))
...
with tab1:
    ...
    if archivo:
        ...
        st.warning(f"⚠️ Esto reemplazará todos los saldos de apertura del año **{anio_fiscal}** y recalculará el Libro Mayor.")
